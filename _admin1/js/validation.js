...
function Activation(n){
	...
	xhttp.open('get','lib/facility_aj.php?q='+n,true);
	xhttp.send();
}
//////////////////Dective///////////////////////////
function deactivation(n){
		var xhttp=new XMLHttpRequest();
	xhttp.onreadystatechange=function(){
		if(xhttp.readyState==4 && xhttp.status==200){
		var del=document.getElementById(n);
	...
	xhttp.open('get','lib/facility_aj1.php?q='+n,true);
	xhttp.send();
}
/////////////////////////facality delete////////////////////
function fac_delete(id){
	if(confirm("Do you Want to Delete")){
		this.document.getElementById("act").value="facality_delete";
		this.document.getElementById("f_id1").value=id;
		//console.log(this.document.getElementById("faculity"))
		this.document.getElementById("faculity").submit();	
	}
}
////////////////////////marks ////////////////////////
function find_sub1(txt){
	var xhttp=new XMLHttpRequest();
	xhttp.onreadystatechange=function(){
		if(xhttp.readyState==4 && xhttp.status==200)
		document.getElementById('m_subject').innerHTML=xhttp.responseText;
	}
	xhttp.open('get','lib/ajax_sub.php?q='+txt,true);
	xhttp.send();
}
///////////////////////Marks total///////////////////
function find_marks(txt){
	var xhttp=new XMLHttpRequest();
	xhttp.onreadystatechange=function(){
		if(xhttp.readyState==4 && xhttp.status==200){
		let t=xhttp.responseText;
		let m=t.split("-");
		document.getElementById('m_total').value=m[0];
		document.getElementById('m_practical').value=m[1];
		}
	}
	xhttp.open('get','lib/ajax_marks.php?q='+txt,true);
	xhttp.send();
}
//////////////////validation for marks/////////////////
function valid_marks(m){
	//alert(m.m_obtain.value);
if((m.m_total.value)<(m.m_obtain.value)){
	alert("Please enter correct marks ");
	return false;
	}
	if((m.m_practical.value)<(m.m_pobtain.value)){
	alert("Please enter correct marks ");
	return false;
	}
	return true;
...
//----------------------------------Admin Logout-----------------------
	function admin_logout(){
		if(confirm("Do you want to logout...!")){
			this.document.getElementById("header").submit();
			}
		}